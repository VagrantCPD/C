...
#define FLEX_LWIP_HANDLE_THREAD	2
...
static pthread_mutex_t lwip_lock = PTHREAD_MUTEX_INITIALIZER;
...
out:
	flex_ipc_return(ipc_msg, ret);
}

void* flex_lwip_handler(void * args)
{
	flex_lwip_handler_arg_t * handler_args = (flex_lwip_handler_arg_t *) args;
	u64 server_shm_addr = handler_args->server_shm_addr;
	u64 client_badge = handler_args->client_badge;
	free(args);
	...
	/* set aff */
	CPU_ZERO(&mask);
	CPU_SET(usys_get_cpu_id(), &mask);
	sched_setaffinity(0, sizeof(mask), &mask);
	sched_yield();

	while(true){
		for(int i = 0; i < MAX_FLEX_IPC_COUNT; ++i){
			ipc_msg_t *ipc_msg = (ipc_msg_t *) (server_shm_addr + i * IPC_PER_SHM_SIZE);
			struct lwip_request *lr = (struct lwip_request *)ipc_get_msg_data(ipc_msg);
			if(lr->req_state == LWIP_REQ_SUBMITTED){
				lr->req_state = LWIP_REQ_BUSY;
				lwip_dispatch(ipc_msg, client_badge);
			}
		}
	}
}

void flex_lwip_main(u64 server_shm_addr, u64 client_badge)
{
	for(int i = 0; i < FLEX_LWIP_HANDLE_THREAD - 1; ++i){
		flex_lwip_handler_arg_t * args = malloc(sizeof(flex_lwip_handler_arg_t));
		args->server_shm_addr = server_shm_addr;
		args->client_badge = client_badge;
		pthread_t tid;
		pthread_create(&tid, NULL, flex_lwip_handler, args);
	}
...
int main(int argc, char *argv[], char *envp[])
{
	...
	info("register server value = %u\n",
	     ipc_register_server(flex_lwip_main,
				 FLEX_CLIENT_REGISTER_HANDLER));

	// info("register server value = %u\n",
	//      ipc_register_server(flex_lwip_dispatch_coroutine,
	// 			 FLEX_CLIENT_REGISTER_HANDLER));
