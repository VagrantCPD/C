...
void *handle_coroutine(void *arg)
{
	char large_buffer[NET_BUFSIZE];
	struct iovec iosend[1];
	struct sockaddr_in client_addr;
	...
	if (accept_fd % 5 == 1) { /* Use send msg */
		...
		ret = sendmsg(accept_fd, &msgsend, 0);
		// gettimeofday(&end, 0);
		// send_time += end.tv_sec - start.tv_sec + ((double) (end.tv_usec - start.tv_usec)) / 1000000;
		// ++send_count;
		chcore_assert(ret == sizeof(large_buffer),
			      "[server] send return error!");
	} else if (accept_fd % 5 == 2) {
	...
	// printf("send operation, total time: %f, total count: %d\n", send_time, send_count);
	return 0;
}

void *accpet_coroutine(void *arg)
{
	int server_fd = 0;
	int ret = 0;
	int accept_fd = 0;
	...
	do {
		...
		create_coroutine(handle_coroutine, (void *)(long) accept_fd);
	} while (1);
...
void run_test_coroutine()
{
	...
	create_coroutine(accpet_coroutine, (void *)NULL);
