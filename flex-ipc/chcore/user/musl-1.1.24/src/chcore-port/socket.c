...
{	
	...
	ipc_msg =
	    flex_ipc_create_msg(lwip_ipc_struct, sizeof(struct lwip_request), 0);
	...
	ret = flex_ipc_call(lwip_ipc_struct, ipc_msg);
	if (ipc_msg_p)
		*ipc_msg_p = ipc_msg;
	else
...
static int chcore_socket_read(int fd, void *buf, size_t count)
{
	...
	if (count > LWIP_DATA_LEN) {
		...
		while (remain > 0 && ret == len) {
			...
			lr_ptr->args[1] = len;
			if ((ret = flex_ipc_call(lwip_ipc_struct, ipc_msg)) < 0) {
				ret = bias > 0 ? bias : ret;
				goto out;
			}
...
int chcore_sendmsg(int fd, const struct msghdr *msg, int flags)
{
	...
	ipc_msg = flex_ipc_create_msg(lwip_ipc_struct, sizeof(struct lwip_request),
