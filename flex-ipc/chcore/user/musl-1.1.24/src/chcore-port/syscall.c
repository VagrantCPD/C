				 ...
				 u64 server_shm_addr, int flex_ipc)
{
	chcore_syscall6(CHCORE_SYS_ipc_register_cb_return, server_thread_cap,
		server_shm_addr, flex_ipc, 0, 0, 0);
}

u64 usys_flex_ipc_send_cap(u32 conn_cap, u64 ipc_msg_ptr, u64 cap_num)
{
	return chcore_syscall3(CHCORE_SYS_flex_ipc_send_cap, conn_cap, ipc_msg_ptr, cap_num);
}
