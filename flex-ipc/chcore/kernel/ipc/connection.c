				...
				u64 server_shm_addr, int flex_ipc)
{
	...
	if(flex_ipc == 1){
		ipc_server_handler_thread->thread_ctx->cpuid = (smp_get_cpu_id() + 1) % PLAT_CPU_NUM;
		ipc_server_handler_thread->thread_ctx->affinity = (smp_get_cpu_id() + 1) % PLAT_CPU_NUM;
		arch_set_thread_arg(ipc_server_handler_thread, server_shm_addr);
		arch_set_thread_badge_arg(ipc_server_handler_thread, conn->client_badge);
		sched_enqueue(ipc_server_handler_thread);
	}
...
u64 sys_flex_ipc_send_cap(u32 conn_cap, ipc_msg_t *ipc_msg_in_client, u64 cap_num)
{
	struct ipc_connection *conn;
	int r = 0;
