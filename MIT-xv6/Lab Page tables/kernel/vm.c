...
// Switch h/w page table register to per process's kernel page table,
// and enable paging.
void
proc_kvminitart(pagetable_t pagetable)
{
...
// free per process's kernel page table
// no need to free leaf memory page
void
...
// copy process's user page table mapping into its kernel page table 
int             
uvmcopy_user_to_kernel(pagetable_t upgtbl, pagetable_t kpgtbl, uint64 oldsz, uint64 newsz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    ...
    for(i = oldsz; i < newsz; i += PGSIZE){
      ...
      pa = PTE2PA(*pte);
      flags = PTE_FLAGS(*pte);
      if(proc_mappages(kpgtbl, i, PGSIZE, (uint64)pa, (flags & (~PTE_U))) != 0){
        goto err;
      }
    }
  }
